...
def RANSAC(model_func, eval_func, data, num_points, num_iter, threshold, recalculate=False):
    ...
    # Recalculate using current consensus set? final refinement
...
def adaRANSAC(model_func, eval_func, data, num_points, threshold, recalculate=False):
    ...
    while k < num_iter:
        ...
        if len(consensus_idx) > max_consensus:
            ...
            p_inleir = max_consensus / len(model_error)
            new_est_max_iters = int(logpd / np.log(1-p_inleir**num_points))
